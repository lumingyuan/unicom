package com.jaemobird.jaemobirdpermissions;
...
/** JaemobirdPermissionsPlugin */
public class JaemobirdPermissionsPlugin implements MethodCallHandler, PluginRegistry.RequestPermissionsResultListener {
  ...
  public static void registerWith(Registrar registrar) {
    final MethodChannel channel = new MethodChannel(registrar.messenger(), "jaemobird_permissions");
    JaemobirdPermissionsPlugin permissionsPlugin = new JaemobirdPermissionsPlugin(registrar);
    channel.setMethodCallHandler(permissionsPlugin);
    registrar.addRequestPermissionsResultListener(permissionsPlugin);
  }

  private JaemobirdPermissionsPlugin(Registrar registrar) {
  ...
  private void requestPermission(String permission) {
    ...
    Log.i("JaemobirdPermissions", "Requesting permission : " + permission);
  ...
  private boolean checkPermission(String permission) {
    Activity activity = registrar.activity();
    permission = getManifestPermission(permission);
    Log.i("JaemobirdPermissions", "Checking permission : " + permission);
    ...
    if (requestCode == 0 && grantResults.length > 0) {
      if (ActivityCompat.shouldShowRequestPermissionRationale(registrar.activity(), permission)) {
        ...
        if (ActivityCompat.checkSelfPermission(registrar.context(), permission) == PackageManager.PERMISSION_GRANTED) {
          ...
          Log.e("JaemobirdPermissions", "set to never ask again" + permission);
          status = 4;
        }
      }
    }
    Log.i("JaemobirdPermissions", "Requesting permission status : " + status);
